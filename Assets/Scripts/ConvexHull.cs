...
{
    public static List<Vector2Int> ComputeConvexHull(List<Vector2Int> points, bool sortInPlace = false)
    {
        if (!sortInPlace)
        ...
        CircularList<Vector2Int> hull = new CircularList<Vector2Int>();
        int L = 0, U = 0; // size of lower and upper hulls

        // Builds a hull such that the output polygon starts at the leftmost Vector2Int.
        for (int i = points.Count - 1; i >= 0; i--)
        {
            Vector2Int p = points[i], p1;
            ...
            if (U != 0) // when U=0, share the Vector2Int added above
                hull.PushFirst(p);
            U++;
    ...
    private static Vector2Int Sub(this Vector2Int a, Vector2Int b)
    {
        return a - b;
    }

    private static float Cross(this Vector2Int a, Vector2Int b)
    {
