Shader "Cel/ExampleOfTheCavityImGoingFor"
{
    Properties
    {
        _DitherPercent("Percent Dithered", Range(0, 1)) = 0
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            // Hash function for generating a pseudo-random number from a seed value
            float hash(float n) {
                return frac(sin(n) * 43758.5453);
            }

            // Function for generating a random number in the range [0, 1] from a seed value
            ...
            fixed4 frag(v2f i) : SV_Target
            {
                ...
                i.pos.xy = floor(i.pos.xy * .25) * .5;
                float checker = -frac(i.pos.r + i.pos.g);

                float randomForPixel = random(i.pos.x * 7 + i.pos.y * 31);
                clip(randomForPixel - _DitherPercent);
